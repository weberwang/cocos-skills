...
`resource_refresh` 必须记录本批资源 URL、`refresh-asset` 请求参数、开始/完成时间和请求结果。`resource_ready_checks` 必须逐项记录资源 URL、`query-asset-info` 的 `imported`、`invalid`、UUID、资源类型和所需子资源，以及 `query-asset-meta` 的 `imported`、UUID 和查询证据。批次含脚本时还必须记录从刷新开始时间起的 `script_console_check`。没有资源写入的纯节点批次可将这些字段记为 `not-applicable`；包含资源写入的批次不得为空。
...
- 每个包含资源创建或导入的批次必须先完成该批全部资源写入，再对公共目录发送一次 `refresh-asset` 请求并等待 Promise 完成。刷新完成前禁止绑定、使用该批资源或开始下一批；不得按文件重复主动刷新，也不得用固定等待时间替代请求完成和状态查询。
- 能力快照必须提供 `refresh-asset`、`query-asset-info` 和 `query-asset-meta`；脚本资源批次还必须提供编辑器控制台错误读取。缺少任一适用能力时批次为 `blocked`，不得执行资源写入或刷新。
- 资源只有在 `query-asset-info` 返回 `imported: true`、`invalid: false`、有效 UUID/类型/所需子资源，并且 `query-asset-meta` 返回 `imported: true`、相同 UUID 后才视为 `ready`。脚本批次还必须证明刷新开始后没有对应编译、加载或同名类错误。执行后发生请求失败、查询超时、结果无效、UUID 不一致、子资源缺失或脚本错误时整批 `failed`，后续绑定不得执行。
