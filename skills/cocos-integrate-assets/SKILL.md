...
3. Call `/health` and `tools/list` or `/capabilities` and save the current capability snapshot. 资源批次要求快照明确提供对应的资源创建/导入、`asset-db` 的 `refresh-asset`、`query-asset-info` 和 `query-asset-meta` 能力；脚本资源批次还必须提供编辑器控制台错误读取能力。缺少任一适用能力时返回 `blocked`，不得开始该批次。节点或绑定批次还必须具备对应查询、组件写入和保存能力。Never guess tool names or parameters.
...
4. 在一个资源批次中创建或导入该批全部计划资源，但不得立即绑定或使用。批次写入完成后，通过能力快照中明确映射到 `Editor.Message.request('asset-db', 'refresh-asset', url)` 的 MCP 操作，对批次公共目录发送一次刷新请求并等待完成；记录请求 URL、批次资源和开始/完成时间。不得执行任意脚本，不得按文件重复主动刷新；编辑器自动触发的刷新不计为代理重复调用。
5. 刷新请求完成后，对每项资源有界轮询 `query-asset-info`，直到返回非空结果且 `imported: true`，同时要求 `invalid: false`、UUID、类型及计划要求的 `subAssets` 均有效；再调用 `query-asset-meta`，要求返回非空且 `imported: true`、UUID 非空并与资源信息一致。图片需要 SpriteFrame 时必须确认对应子资源已生成。查询超时、结果无效或 UUID 不一致时整批 `failed`。
6. 批次包含脚本时，在资源与 Meta 检查后读取从刷新开始时间起的编辑器控制台错误；存在该批脚本的编译、加载或同名类错误时整批 `failed`。等待和检查期间不得绑定资源、保存含这些资源引用的场景或开始下一批。任一资源未就绪即整批失败，不得使用该批其他资源绕过门禁。
...
8. Save the current scene and immediately read back every node, component, property, and asset reference. Run that batch's `readback_checks`，并确认绑定引用仍等于资源就绪检查读取的 UUID/子资源 ID。On failure, stop later batches, preserve evidence, and return `failed`; do not continue with improvised repair.
9. Start the next batch only after the previous batch's applicable resource-ready, script-console and binding read-back gates pass；标为 `not-applicable` 的门禁不阻塞纯节点批次。Write the prototype or global release integration report, scene read-back evidence, and result when complete; the orchestrator performs state transition. Formal scene loops are already complete before release integration starts.

## Result and invalidation

Return frozen input versions and hashes, changed paths, capability snapshot, per-batch commands or methods, `refresh-asset` completion, asset-info/asset-meta/sub-resource readiness evidence, script-console checks when applicable, import and binding read-backs, scene-save evidence, issues, and unblock conditions. Any change to code, assets, scenes, plan, requirements, or visual direction makes prior integration, verification, build, and delivery results `stale`; rerun required batches.
