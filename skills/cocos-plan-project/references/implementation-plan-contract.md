...
vertical_slice:
  ...
  task_ids: [] # 原型路径：core-gameplay-code → prototype integration → 人工审核 → vertical-slice-review
...
scene_loops:
  - id: scene-loop-menu
    ...
    scene_boundary_task_id: menu-scene-boundary-grilling
    task_ids: [] # scene-boundary-grilling → pencil-draft → visual-concept（原画候选、精确 UI、精修与质量门槛）→ asset-preparation / code → human-review
...
4. **推进到玩法场景时按正式版本实现**：若 `scene_loop.is_core_gameplay: true`（或其 `scene_id` 属于 `vertical_slice.scene_ids`），必须走完整正式小循环 `scene-boundary-grilling → pencil-draft → visual-concept → asset-preparation / code → human-review`，用正式版替换原型，不得直接把原型当作交付物。全局 `integration` 与 `verification` 只在全部正式循环通过后执行。
...
- 允许 `kind: core-gameplay-code`、对应 `integration`、人工垂直切片审核与 `vertical-slice-review`。
...
`global_scaffold` 是唯一的 `kind: global_scaffold` 前置任务，必须在核心玩法确认与 `module_decomposition` 通过后、所有正式 `scene_loops` 前完成。它只拥有共享启动路径，并提供应用入口、场景路由、全局状态/事件、配置、资源服务、错误边界与可测试接口。`global_scaffold_task_id` 必须被每个正式场景循环引用；其 `acceptance_checks` 未通过时，禁止正式场景代码、集成和人工验证。

## 正式场景小循环

`scene_loops` 必须覆盖每个可交付 `scene_id`，且全部为正式版本实现。每项包含稳定 `id`、`scene_id`、`business_flow_level`、`is_core_gameplay`、`depends_on`、唯一 `scene_boundary_task_id`、按 `scene-boundary-grilling → pencil-draft → visual-concept → asset-preparation / code → human-review` 排列的 `task_ids`，以及非空 `exit_checks`。全局集成与人工验证不属于场景循环。
...
- 每个正式循环必须先执行同场景、只读的 `scene-boundary-grilling`。它以该场景的 `scene_blueprint`、需求、系统设计、技术设计、实施计划和冻结视觉方向为输入，输出 `artifacts/scene-boundaries/<scene_id>.md`；工件必须列明场景目的、进入/退出条件、玩家动作、UI 状态、数据输入/输出/持久化、模块/事件/节点边界、失败/空态/返回路径、非范围及验收标准，并具有与 `content_hash` 绑定的用户确认。未决问题、跨场景归属不明或缺少确认时，不得启动 Pencil 草图。
- `pencil-draft` 必须直接依赖已批准的 `scene-boundary-grilling`，记录边界哈希、人工批准的草图哈希与功能视觉检查；`visual-concept` 必须依赖该草图，继承同一边界哈希，并逐项绑定全局视觉冻结的版本、内容哈希、两张参考效果图、颜色 token、克制/发散 profile、功能 UI 规则和通过的质量评分。没有这些通过证据不得启动本场景的资源准备或正式代码任务。
...
- 每个 `visual-concept` 任务只能声明一个 `scene_id` 和一个最终图输出路径，并包含 `single-scene-scope` 检查。正式循环按 `scene_loops` 声明顺序串行：除首个场景外，每个场景的 `scene-boundary-grilling` 必须直接依赖前一个场景的 `human-review`；不得提前拷问后续场景或并行生成多个页面效果图。
- 只有本循环 exit_checks 的项目内证据全部通过，才可启动下一场景循环；共享模块必须在首个消费者循环前完成。正式循环不得调用 Cocos MCP 写入；唯一 Cocos 写者仅在后续全局 integration 阶段执行。

## Capture manifest

规划阶段必须同时生成 `.cocos-workflow/artifacts/capture-manifest.yaml`。它固定人工审核的目标、基线和像素差阈值，不能由验证阶段临时删减：
...
review:
  mode: human # 验证只能由人工执行和判定
  initial_scene: assets/scenes/menu.scene # 必须等于冻结 project-profile.initial_scene
  required_reviewer: human
  execution_instruction: 在 Cocos Creator 编辑器中手动启动预览
profiles:
  - profile_id: mobile-small
    ...
    checks:
      - id: home-start
        steps: [{action: human-open-preview}, {action: human-tap, target: start-button}]
        expected_observations: []
        screenshot_path: reports/human-review/mobile-small/home-start.png
...
- `review.mode` 固定为 `human`，`initial_scene` 必须等于冻结项目配置的 `initial_scene`，且 `required_reviewer` 为非空人工身份。每个 check 只列人工在 Cocos Creator 编辑器预览中的操作和预期观察；自动启动、自动交互、自动截图、Chrome 控制、MCP 调试调用与自动判定一律禁止。
...
- `tasks` 每项遵守总控任务契约并额外含 `kind`（`core-gameplay-code | module_decomposition | global_scaffold | pencil-draft | visual-concept | scene-boundary-grilling | code | asset-preparation | human-review | integration | vertical-slice-review`）与可选正整数 `business_flow_level`。每项必须声明符合团队角色路由的 `role`、当前 `role_prompt_version` 和唯一 `skill`；角色代表稳定团队职责，`kind` 不得被扩张为新角色。所有任务必须有唯一 `task_id`、非空 `depends_on`（核心玩法代码与垂直切片审阅的首个任务除外）和 `allowed_paths`。
- 原型任务：`core-gameplay-code` 与其集成/验证任务必须声明 `scene_id` 且属于 `vertical_slice.scene_ids`；不得声明正式 `module_ids` 依赖。
- 正式任务：每个 `pencil-draft`、`visual-concept`、`scene-boundary-grilling`、`code` 与 `asset-preparation` 必须声明 `scene_id` 与 `business_flow_level`。`scene-boundary-grilling` 必须依赖该场景循环的垂直切片审阅、模块拆分、全局骨架及前置业务流完成门禁（如适用），且只拥有 `artifacts/scene-boundaries/<scene_id>.md`、结果和报告路径；`pencil-draft` 必须直接依赖同场景 `scene-boundary-grilling` 并在 `inputs` 中记录边界内容哈希；`visual-concept` 必须依赖同场景 `pencil-draft` 并继承该边界哈希；`code` 与 `asset-preparation` 必须直接依赖同场景 `visual-concept`，并在 `inputs` 中记录同一场景边界的内容哈希、Pencil/高保真批准哈希、全局视觉版本、内容哈希及两张参考效果图。每个正式 `code` 还必须依赖 `module_decomposition` 与 `global_scaffold`。每个正式循环必须以 `human-review` 收口。`integration` 只允许位于全局 integration 阶段，按 `batch_index` 串行并依赖全部 `business_flow_levels` 的完成门禁。
- `path_ownership.production_writers` 中每个任务的可写路径不得重叠；`cocos_writer` 必须是非空单个任务 ID，且所有 integration 任务均引用它。
- `integration_batches` 每项含 `batch_index`、`task_ids`、`scene_blueprint_ids`、`resource_paths`、`refresh_required`、`resource_ready_checks`、`script_console_check_required`、`readback_checks`；批次序号连续，每批结束均要读回验证。每个正式场景的节点/组件落地批次必须先于该场景的资源导入与绑定批次，且两类批次都不能省略。包含资源创建或导入的批次必须设置 `refresh_required: true`，并声明等待 `refresh-asset` 完成，随后用 `query-asset-info` 和 `query-asset-meta` 校验导入状态、UUID、资源类型及所需子资源；包含脚本时还必须设置 `script_console_check_required: true`。能力缺失时批次 `blocked`，绑定只能位于同批资源就绪门禁之后。
