...
- `tasks` 每项遵守总控任务契约并额外含 `kind`（`core-gameplay-code | module_decomposition | global_scaffold | pencil-draft | visual-concept | scene-boundary-grilling | code | asset-preparation | human-review | integration | vertical-slice-review`）与可选正整数 `business_flow_level`。所有任务必须有唯一 `task_id`、非空 `depends_on`（核心玩法代码与垂直切片审阅的首个任务除外）和 `allowed_paths`。
