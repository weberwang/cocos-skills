...
2. Read `.cocos-workflow/project-profile.yaml`, `requirements.yaml`, approved `systems-design.yaml`, approved `technical-design.yaml`, the frozen visual-direction artifact, and approved scene concepts. Require approved project configuration, requirements, systems design, technical design, visual direction, and concepts.
3. Compare the current requirements、系统设计、技术设计哈希，`visual_direction.version`、`visual_direction.content_hash` 和概念哈希与批准记录。On any missing or mismatched input, return `blocked` or `stale`; do not create an executable plan.
...
- If requirements, systems design, technical design, visual direction, concepts, or project-profile hashes change, reject plan reuse and mark the plan and downstream outputs `stale`.
...
- `implementation-plan.yaml` must carry frozen input versions and `sha256:` hashes and define scenes, prefabs, scripts, asset dependencies, `module_decomposition`, `dependency_graph`, `scene_loops`, `vertical_slice`, tasks, ownership, the asset-approval join gate, and the one Cocos writer.
