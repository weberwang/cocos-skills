...
9. Define formal 场景小循环 (`scene_loops`): each scene loop owns one `scene_id` and `business_flow_level`, sets `is_core_gameplay` when the scene is in `vertical_slice.scene_ids`, depends on `vertical-slice-review` + `module_decomposition` + `global_scaffold_task_id`, and orders `pencil-draft → visual-concept → asset-preparation → code → human-review` inside `production`. Each `visual-concept` task owns exactly one scene and one final image; order these tasks by `scene_loops`, and make every later visual task depend on the prior scene's human review so effect images are never batch-generated. When a loop is core gameplay, it is the **formal version** that replaces the prototype—never ship the prototype as the deliverable. The Pencil draft must be approved before the high-fidelity concept; the concept must bind the frozen visual version/hash, both global reference-effect images, color-token system, restraint/expression profile, functional-UI rules and passed quality rubric before asset preparation or scene/UI code starts. Release the next scene only after all `exit_checks` pass. After every formal loop is complete, schedule one global `integration` task, then one global Chrome `verification` task. Only the global integration task may write through Cocos MCP.
...
- `implementation-plan.md` must carry frozen input versions and `sha256:` hashes in YAML front matter and explain the fixed production order, `vertical_slice` prototype gate, `business_flow_levels`, scenes, prefabs, scripts, approved Pencil-draft and high-fidelity-concept dependencies, asset dependencies, `module_decomposition`, `dependency_graph`, formal `scene_loops` (including `is_core_gameplay`), tasks, ownership, the per-scene asset-approval join gate, and the one global Cocos writer in its body. It must not carry mutable task status, result, or runtime approval fields.
