...
description: Use when a Cocos Creator 2D Web Mobile project verified by a human reviewer needs a local Web Mobile build or final delivery package, including artifact integrity checks, file hashes, manifest, delivery report, and human-gate-preserving handoff.
---
...
- `entry_mode=build`：仅用于 `building` 阶段。构建 Web Mobile、生成构建清单和哈希并校验入口与资源清单；不启动游戏、不创建静态预览，也不创建最终压缩包或标记交付完成。游戏运行验收只在上游人工验证阶段进行。
...
5. 仅对构建目录执行非运行时完整性检查：入口文件、清单引用的资源、文件哈希、路径安全性和构建日志必须完整。不得启动本地静态服务、通过 Chrome 打开构建目录或把构建产物当作游戏运行证据。
...
- 禁止运行或预览编译/打包产物；游戏运行、交互和视觉验证只能由人工审核者完成。构建/打包阶段仅验证交付物完整性，不能替代上游人工验证。

## 交接

返回总控结果契约所需的基线、冻结输入版本/哈希、产物、完整性检查、证据、问题与交接说明。只有 `build` 模式的构建与完整性检查均通过，才能让总控进入 `delivery`；只有 `package` 模式的清单、压缩包、校验和与报告齐全，才能请求最终人工门禁，不能自行完成流程。
