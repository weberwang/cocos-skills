...
implementation_mode: formal # prototype | formal；原型仅用于核心玩法确认
...
scene_boundaries: [] # formal 时为 {scene_id, path, content_hash}；原型为空
...
bindings:
  - binding_id: game-controller
    ...
    scene_boundary_hash: sha256:<approved scene boundary>
    properties: []
    asset_ids: []
    binding_order: 1
    readback_assertions: []
...
- `formal` 模式的每个场景必须在 `scene_boundaries` 中记录已批准 `artifacts/scene-boundaries/<scene_id>.md` 的路径与内容哈希；每个 binding 的 `scene_boundary_hash` 必须精确匹配该场景条目。原型模式不得伪造正式边界工件。
