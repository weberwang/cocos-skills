...
4. Code-only tasks may execute in parallel with asset generation when path ownership allows. Before writing any binding entry with a nonempty `asset_ids`, read `artifacts/game-assets/<scene_id>.yaml` for that binding's scene and require `status: approved`, a valid human approval whose `subject_hash` equals its `content_hash`, and exact asset IDs. Prototype tasks may leave asset bindings empty or use planned placeholders; formal tasks must not insert unapproved asset IDs.
5. For formal work: execute the sole `kind: global_scaffold` task immediately after approved module decomposition and vertical-slice confirmation. Do not start scene-specific formal `kind: code` until scaffold acceptance checks pass and that scene's Pencil-draft, high-fidelity concept and `artifacts/scene-boundaries/<scene_id>.md` approvals pass. 场景边界工件必须无未决问题，且确认与审批主题哈希均等于其 `content_hash`；只实现其中明确归属当前场景或当前模块的功能。对场景代码，读取同场景 `scene_blueprint`，只允许引用蓝图中已声明的稳定节点 ID、组件和事件挂载点；缺失场景根、运行时根、Canvas/UI 根、游戏内容根或任务需要的组件时返回 `blocked`，不得用代码补造编辑器节点。Claim only code tasks whose `allowed_paths` belong to this worker and whose `module_ids` are in approved `module_decomposition`. Return path overlaps, undeclared modules, missing `global_scaffold` dependency, missing design approvals, missing scene-boundary approval, missing blueprint, or cyclic `dependency_graph` entries to the orchestrator; do not partition or take ownership unilaterally.
...
4. Generate `.cocos-workflow/artifacts/code-binding-manifest.yaml` with script path, class, target scene or prefab, stable node ID, component, properties, asset dependencies, binding order, and read-back assertions. 每个正式场景绑定必须引用 `scene_boundary_hash`、`scene_blueprint_id` 与已声明节点 ID，并声明所依赖的节点/组件读回断言；不得把节点名或层级路径当作稳定 ID 的替代品，也不得绑定边界工件未声明的功能。它声明集成意图；它不是编辑器证据。Prototype manifests must mark `implementation_mode: prototype`.
