...
2. Verify `project_profile_hash`, `requirements_hash`, visual-direction `version` and `content_hash`, and plan `content_hash` against current frozen inputs. For a scene or UI code task, also require its approved Pencil-draft and high-fidelity concept records, including the two frozen-reference image hashes. On mismatch, mark the result `stale` and do not change code.
