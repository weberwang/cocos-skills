...
review: {mode: human, initial_scene: assets/scenes/menu.scene, reviewer: <human identity>, review_record: reports/human-review/vertical-slice.yaml}
scene_ids: []
formal_scene_loop_ids: [] # 后续正式版本实现对应的 scene_loop id
core_loop_evidence:
  start: reports/human-review/mobile-standard/slice-start.png
  challenge: reports/human-review/mobile-standard/slice-challenge.png
  resolution: reports/human-review/mobile-standard/slice-resolution.png
profiles: []
playtest_evidence: []
...
- `review.mode` 必须为 `human`，审核记录的初始场景、人工身份和签署结论必须与本次验证一致；自动运行、自动交互、构建产物或本地静态服务都不能作为切片证据。
