...
3. Build each asset prompt from the approved final concept, selected game-art layer, editable UI source and frozen direction. 按“资源用途、关键视觉特征、必要技术约束、自由空间”组织提示词，只保留会影响识别、风格一致性、尺寸或运行时用途的信息；不要转录全部上游工件、堆砌形容词、逐项描述非关键细节或混入矛盾要求。Use the asset plan's exact output dimensions; preserve the source's decisive silhouette, material, light direction, edge treatment and active color-token roles, while leaving non-critical surface detail to the model. Generate artwork with `$imagegen`; UI文字、关键图标和几何组件优先从可编辑设计源精确导出，不得用重新生成近似图替代。仅在资源用途要求时使用透明背景。
