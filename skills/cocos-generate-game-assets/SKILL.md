...
3. Build each asset prompt from the approved concept and frozen direction. Use the asset plan's exact output dimensions; preserve color-token roles and usage rules, the approved restraint/expression profile, palette, style, and prohibited list from the visual direction and do not introduce a local visual variation. Generate with `$imagegen`; request transparent background only when the declared asset purpose requires it.
