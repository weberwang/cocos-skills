...
1. 验证项目配置、需求和系统设计的批准记录与哈希；任一不一致即返回 `blocked` 或 `stale`。若任务声明会修改已批准技术设计的 `decision_change`，还必须校验同阶段、同主题哈希的 `$grilling` 确认；缺失或不匹配时返回 `blocked`。
2. 记录 Cocos Creator 版本约束、场景/模块边界、数据流、事件边界和资源加载策略；边界必须能支撑 MVP 系统，不能以未验证 API 作为前提。对每个可交付场景定义 `scene_structure`：场景根、运行时根、Canvas/UI 根、游戏内容根、HUD/弹层层级、安全区容器，以及按交互需要声明的输入/相机节点。每个节点必须有稳定 ID、父节点、职责、必需组件和关键属性；组件类型只能使用该 Creator 版本与能力快照可确认的名称。
...
- 已批准技术设计的决策性返工未通过 `$grilling` 门禁时，不得写入或提交新的技术设计工件。
- 至少一个 ADR 必须覆盖每个标记为 `high` 的技术风险；未知 Creator API 只能作为待验证风险，不能伪装为已确认能力。
- `scene_structure` 必须覆盖所有可交付场景，并明确哪些节点为场景必需节点、哪些为条件节点及其启用条件；不得只写场景名称、`node_ids` 或脚本列表。
