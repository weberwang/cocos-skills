...
每个场景/UI 的工件为 `.cocos-workflow/artifacts/scene-concepts/<scene_id>.yaml`，图像与记录位于 `.cocos-workflow/art/concepts/<scene_id>/`。工件只包含一个 `scene_id`，从而允许无路径冲突的场景循环独立推进。
...
frozen_reference_effect_images: # 必须逐项复制 visual-direction.yaml 中恰好两张参考效果图的 path 与 content_hash
...
pencil_draft:
  artifact_path: artifacts/pencil-drafts/home.yaml
...
- `frozen_reference_effect_images` 必须恰好两项，并与 `visual-direction.yaml.reference_effect_images` 的路径和内容哈希逐项一致；不一致时整份清单为 `stale`。
