...
2. 编写单场景设计简报，明确叙事时刻、玩家视觉动线、前/中/后景、角色与环境关系、UI 信息优先级、真实文案、组件状态、触控与安全区约束。选择与已批准页面类型对应的冻结克制/发散 profile；没有匹配项时只能使用 `default`，不得临时改比例。只使用已批准内容。
3. 先制作游戏原画层。按提示词模板将每个请求压缩为“目标、画面重点、视觉方向、必要约束、自由空间”，只写影响构图、风格、用途或验收的关键信息；不得堆砌辞藻、重复冻结工件、罗列无关细节或同时给出矛盾要求。用 `$imagegen` 生成至少 3 个构图真正不同的候选，不得只改颜色或随机种子；每次生成必须将两张冻结质量锚点作为实际参考图输入，并为该候选保存单独的生成请求记录，逐项记录角色、路径、哈希、请求与输出绑定。候选分别探索焦点、镜头、纵深和光影，但必须保持冻结风格；未冻结的次要细节应留给模型自由发挥。原画层不得包含 UI、伪文字、设备框、拼贴或水印。
4. 对候选执行盲评式设计审查并保存评分与缺陷：构图叙事、造型与剪影、透视纵深、材质光影、色彩控制、克制/发散平衡、风格一致性各 1–5 分。淘汰任何单项低于 4 分的候选，记录入选理由；若无候选通过，必须重新生成，不得进入合成。
...
7. 执行质量门槛：冻结一致性、Pencil 结构、构图叙事、原画完成度、UI 层级、文字可读性、组件状态、触控与安全区、视觉噪声、克制/发散平衡、资产可拆分性。每项 1–5 分，全部不低于 4 且平均分不低于 4.5；主/次焦点数量及高饱和、高对比、重材质和强动效只能出现在冻结 profile 授权的发散区域。另在 100% 尺寸和三个捕获视口检查真实文案无乱码、截断、重叠或低对比。未达标必须定向修正并重新评审，禁止以人工豁免绕过。
8. 将最终图与两张冻结参考效果图并排呈现给人工审核。只有明确批准且 `approval.subject_hash == content_hash` 时，才将场景工件设为 `approved`；`final_image_hash` 作为审核证据记录。被拒绝的场景在相同冻结输入下继续迭代。
