...
3. Derive a stable `scene_id` from each in-scope requirement page. Select the matching frozen restraint/expression context, or the frozen default when no approved context applies; never invent a per-scene ratio. Write a prompt record for each scene using [the prompt template](references/mobile-scene-prompt-template.md). Copy the frozen direction vocabulary, commercial benchmark patterns, color-token rules, restraint/expression profile, functional-UI rules, quality rubric, two frozen reference-effect images, and exact mobile resolution verbatim; only add content that is already present in the approved Pencil draft and requirement page.
4. Generate an effect image for each scene with `$imagegen` only after its Pencil draft is explicitly approved. Preserve the declared primary action and information hierarchy. Do not use phone hardware frames, device mockups, unreadable fake text, unrelated watermarks, a collage of multiple screens, copied commercial brands, or a locally invented palette unless the approved requirement explicitly asks for them.
5. Store the image, Pencil-draft path/hash and approval, prompt record, generator metadata, frozen-reference image hashes, source hashes, and binary SHA-256 in that scene directory. Record visual defects, text limitations, functional-UI risks, and any manual-edit need rather than silently claiming fidelity.
6. Run the contract checks for every scene: Pencil-structure consistency, exact visual version/hash binding, color-token compliance, restraint/expression budget and focal-count compliance, functional-UI clarity, and a scored human comparison against both frozen reference-effect images. A scene cannot pass if any rubric score is below the frozen minimum or its average is below the frozen threshold. Present the complete set for human review. Until explicit approval, keep each review and the manifest pending; a rejected scene remains failed or blocked and is regenerated under the same frozen inputs.
7. After explicit per-scene approval, set the per-scene artifact to `approved`, bind its hash to the visual version/hash, and return `scene_concept_artifact`, restraint/expression profile, checks, screenshots/preview evidence, Pencil approval evidence, and approval evidence to the orchestrator.
...
- Do not use a generic prompt to compensate for a missing benchmark, color system, functional-UI rule, or quality rubric. Return `blocked` until the frozen direction supplies them.
