...
3. 先制作游戏原画层。用 `$imagegen` 生成至少 3 个构图真正不同的候选，不得只改颜色或随机种子；每次生成必须将两张冻结质量锚点作为实际参考图输入，并为该候选保存单独的生成请求记录，逐项记录角色、路径、哈希、请求与输出绑定。候选分别探索焦点、镜头、纵深和光影，但必须保持冻结风格。原画层不得包含 UI、伪文字、设备框、拼贴或水印。
