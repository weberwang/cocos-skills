...
direction:
  ...
  color_system:
    tokens: [] # token, hex, role, budget_bucket(base|surface|accent), allowed_surfaces, contrast_notes
    usage_rules: []
...
reference_reviews:
  - {role: game-art-quality-anchor, image_path: art/visual-references/<reference_id>.png, image_hash: sha256:<hash>, scores: {}, average_score: 0, verdict: pending, evidence: []}
...
reference_effect_images: # 必须恰好两项；视觉冻结阶段生成的全局风格参考图，不是场景概念图
  ...
  - path: art/visual-references/<reference_id>.png
    ...
    generator: <recorded ImageGen metadata>
    content_hash: sha256:<image binary hash>
    editable_source_path: art/visual-references/<reference_id>.pen
    editable_source_hash: sha256:<editable source hash>
    review_status: pending # pending | approved | rejected
approval:
...
- `color_system.tokens`、`usage_rules` 与 `composition` 必须非空；每个 token 必须有唯一 `budget_bucket`（`base`、`surface` 或 `accent`）和允许使用位置；比例在 0–1 之间且三项之和为 1，`max_simultaneous_accents` 为正整数。每条用量规则必须说明适用 token、页面区域和验证证据。`palette` 中每个颜色必须能追溯到一个 token。
...
- `reference_effect_images.role` 必须恰好包含一次 `game-art-quality-anchor` 和一次 `ui-system-quality-anchor`。UI 锚点必须使用真实文案，并保存位于 `art/visual-references/` 下、扩展名为 `.pen` 或 `.svg` 的可编辑文字与关键图标设计源，以及精确匹配的 `editable_source_hash`；禁止用生成伪文字证明 UI 质量。
- `game_art_system` 与 `ui_system` 的每个字段均须非空且可执行。组件至少声明 normal、pressed、disabled、selected 四种状态；文字层级必须给出字号、行高和对比规则；触控尺寸必须可被项目视口验证。
- `quality_rubric` 的全部七维必须完整；`reference_reviews` 必须恰好两项并逐一对应锚点图，逐项记录 role、图像路径、图像哈希、评分、证据和结论。每个维度不低于最小值、平均分不低于阈值、`verdict` 为 `passed`，否则不得冻结。
- 规范化哈希使用字段名排序的 UTF-8 JSON 表示；排除最外层 `content_hash` 与 `approval.subject_hash`，不丢弃空数组或空映射。
