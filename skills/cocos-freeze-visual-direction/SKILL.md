...
4. 使用 `$imagegen` 生成恰好两张移动端质量锚点：一张 `game-art-quality-anchor`，验证原画构图、材质、光影和完成度；一张 `ui-system-quality-anchor`，验证 UI 层级、组件、图标与真实文案的呈现。每条提示词按“目标、画面重点、视觉方向、必要约束、自由空间”组织，只提炼当前锚点实际使用的颜色 token、克制/发散预算、焦点层级和商业基准抽象模式；不得转录整份规范、堆砌同义辞藻、描述无关微观细节或保留互相冲突的要求。冻结边界外的次要构图与表现手法应留给模型发挥。UI 锚点中的文字和关键图标必须以可编辑设计元素精确重建后导出，不接受生成伪文字。为 UI 锚点保存 `.pen` 或 `.svg` 可编辑源及其哈希，并记录职责、提示词哈希、生成器元数据、二进制哈希和审核状态；此阶段不得生成具体业务场景成品。
