...
2. Build a global visual benchmark before writing prompts. Define 3–5 commercially released games for analysis only; for each, record the observable patterns to borrow and the patterns to avoid. Never copy a screenshot, brand, character, logo, UI layout, or asset. Turn the benchmark into an executable direction: audience fit, visual keywords, color tokens and usage ratios, a restraint/expression budget by screen context, typography, shapes, composition, icon treatment, motion cues, functional-UI rules, accessibility contrast rules, prohibited styles, and traceable source-reference usage.
...
4. Use `$imagegen` to generate exactly two mobile-oriented reference effect images from the proposal. Prompts must name the approved color-token roles, color budget, restraint/expression budget, focal hierarchy, functional-UI rules, and the benchmark's observable patterns; they must exclude copied brands and assets. Use the frozen orientation and design resolution, record each image's purpose, prompt hash, generator metadata, binary SHA-256, and review status, and do not generate scene-specific screens in this phase.
...
6. Score both references against the visual-quality rubric before review. A reference fails if any dimension is below the configured minimum, its average is below the threshold, its primary action or information hierarchy is unclear, its palette violates the approved token budget, or its focal-point count and visual intensity violate the approved restraint/expression budget. Regenerate the failed reference under the same direction rather than silently weakening the score.
...
- Do not treat vague adjectives such as “高级” or “好看” as acceptance criteria. Freeze measurable token, hierarchy, functional-UI, and quality-rubric rules instead.
- Do not freeze if fewer than three commercial benchmarks, a missing color-token usage rule, a missing restraint/expression budget, or an unpassed reference review leaves ImageGen free to invent the visual system.
...
The next phase receives only a `frozen` artifact with `visual_direction_version`, `content_hash`, `orientation`, `design_resolution`, exactly two reference effect images, commercial benchmark, color system, functional-UI rules, passed quality rubric, and valid approval. Every downstream visual task must echo those values exactly; otherwise it is stale.
